...
def test_branch_job_persists_child_node_and_webhook(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(simulation_worker_module, "generate_initial_graph", lambda user_intent, top_k=10: _stub_initial_graph())
    monkeypatch.setattr(
...
def test_queued_simulation_job_is_claimed_after_restart(monkeypatch, tmp_path):
    ...
    row = connection.execute("SELECT status, progress, result_json FROM jobs ORDER BY created_at DESC LIMIT 1").fetchone()
